...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            ...
            frozenset({"bd1b10d9c9facfb4655a5c36d6a17cd4180eb63a5a921f0c4796abf7561f6f85"}),
            id="pdb2xbi",
        ),
    ),
)
def test_add_hydrogens_matches_known_upstream_hydrogen_digests(
    input_path: Path, expected_digests: frozenset[str]
) -> None:
    """Supported repair fixtures should retain verified upstream digests.

    ``pdb2a1d`` contains rotatable-hydrogen cases whose 3-decimal digest can
    differ across platforms while preserving the same permanent regression
    surface. We accept the known stable digests here and keep the higher-level
    semantic checks in the representative regression suite.
    """
